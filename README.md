...
Options:
	...
	-D (upper) ref database
	-m Meta data file [Required]

	[Input options, required]
	  -i Sequence files list, 2bRAD or WGS data [Conflicts with -l]
	  -p List file path prefix [Optional for -i]
	or
	  -l 2bRAD sequence files list [Conflicts with -i]
	  -T (upper) Input Species-level abundance table generated by 2bRAD-M [Conflicts with -i]
	...
	[Profiling parameters]
	  -M (upper) Sequence type, T (2bRAD) or F (Shotgun), default is T
	  -a the abundance threshold of species for Strain2bFunc analysis, default is 0.01
