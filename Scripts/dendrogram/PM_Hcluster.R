...
p <- c("optparse", "ape", "ggplot2", "ggdendro")
...
option_list <- list(
  ...
  make_option(c("-m", "--meta_file"), type="character", help="Input species of genome file [Required]"),
...
## load data
# import dist file
dst_orig <- read.table(file=opts$dist_file, sep = "\t", header=TRUE, row.names=1)
meta <- read.table(file=opts$meta_file, sep = "\t", row.names=1)

rownames(dst_orig) <- paste(meta[rownames(dst_orig), "V2"], rownames(dst_orig), sep = "_")
colnames(dst_orig) <- paste(meta[colnames(dst_orig), "V2"], colnames(dst_orig), sep = "_")
...
PM_hcluster <- function(da, method="average",k=2) {
  ...
  pdf(opts$outfile,width=1+attr(dist.data,'Size')/4,height=10)    # need to solve width and height auto adjusting
