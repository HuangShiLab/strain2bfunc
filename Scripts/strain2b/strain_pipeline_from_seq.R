...
option_list <- list(
	      ...
	      make_option(c("-d", "--cnm_dir"), type="character",
...
if (opts$mode == 0) {
  if (is.null(opts$cnm_dir)) {
    stop("Please specify a directory of copy number matrix!")
  } else {
    Sample_List_Pipeline(opts$sample_list_file, opts$species_list_file, opts$output_path, cnm_dir = opts$cnm_dir, mode = opts$mode)
  }
} else if (opts$mode == 1) {
